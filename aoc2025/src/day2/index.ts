// --- Day 2: Gift Shop ---

const sampleData =
  '11-22,95-115,998-1012,1188511880-1188511890,222220-222224,1698522-1698528,446443-446449,38593856-38593862,565653-565659,824824821-824824827,2121212118-2121212124'

const realData =
  '2157315-2351307,9277418835-9277548385,4316210399-4316270469,5108-10166,872858020-872881548,537939-575851,712-1001,326613-416466,53866-90153,907856-1011878,145-267,806649-874324,6161532344-6161720341,1-19,543444404-543597493,35316486-35418695,20-38,84775309-84908167,197736-309460,112892-187377,336-552,4789179-4964962,726183-793532,595834-656619,1838-3473,3529-5102,48-84,92914229-92940627,65847714-65945664,64090783-64286175,419838-474093,85-113,34939-52753,14849-30381'

const regex = /^((\d)+)\1$/g

function parseRange(range: string) {
  const [firstId, lastId] = range.split('-')
  let count = Number(firstId)
  let invalidIds: string[] = []
  while (count <= Number(lastId)) {
    const reMatch = String(count).match(regex)
    if (reMatch) invalidIds.push(...reMatch)
    count++
  }
  return [...new Set(invalidIds)]
}

const getInvalidIds = (ranges: string[]) => {
  return ranges.flatMap((range) => {
    const result = parseRange(range)
    console.log(
      `- ${range} has ${result.length} invalid IDs;`,
      result.length > 0 ? `(${result.join(', ')})` : ``
    )
    return result
  })
}

export default function () {
  const invalidIds = getInvalidIds(realData.split(','))
  const sum = invalidIds.reduce((sum, id) => sum + Number(id), 0)
  console.log(`=== Sum of invalid IDs: ${sum} ===`)
}
